...
RSpec.describe "メッセージ投稿機能", type: :system do
  ...
  context '投稿に失敗したとき' do
    it '送る値が空の為、メッセージの送信に失敗すること' do
      ...
      # DBに保存されていないことを確認する
      expect{find('input[name="commit"]').click}.to not_to change(Message.count)

      # 元のページに戻ってくることを確認する
      expect(current_path).to eq room_message_path(@room_users.room)
  ...
  context '投稿に成功したとき' do
    it 'テキストの投稿に成功すると、投稿一覧に遷移して、投稿した内容が表示されている' do
      ...
      expect{find('input="commit').click}.to change{Message.count}.by(1)
